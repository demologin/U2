...
public class VectorOld extends Var {

	private final double[] values;

	public VectorOld(double[] values) {
		this.values = values.clone();
	}

	public VectorOld(VectorOld vector) {
		this.values = vector.values.clone();
	}

	public VectorOld(String strVectorValues) {
	...
	public Var sub(VectorOld other) {
		checkOtherVector(other);
		double[] result = this.values.clone();
		for ( int i = 0; i < result.length; i++ ) {
			result[ i ] -= other.values[ i ];
	...
	public Var mul(VectorOld other) {
		checkOtherVector(other);
		double[] result = this.values.clone();
		for ( int i = 0; i < result.length; i++ ) {
			result[ i ] *= other.values[ i ];
	...
	private void checkOtherVector(VectorOld other) {
